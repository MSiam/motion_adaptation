...
def compute_saliency(imSeq, flowSz=100, flowBdd=12.5, flowF=3, flowWinSz=10,
    ...
    for i in range(n):
        ...
        if isFrameDominant > 0:
            ...
            motion_conf.append(True)
        else:
            motion_conf.append(False)
    ...
    return salImSeqOrig, motion_conf
...
def nlc(imSeq, maxsp, iters, outdir, suffix='',
            ...
            redirect=False, doload=False, dosave=None, indices=None):
    """
    ...
    salImSeq, conf = compute_saliency(imSeq, flowBdd=12.5, flowDirBins=20,
                                redirect=redirect)
    for i in range(salImSeq.shape[0]):
        temp= salImSeq[i,:,:]
        temp= cv2.resize(temp, (854,480))
        if temp.max()!=0:
            temp *= 255.0/temp.max()
        cv2.imwrite(outdir+'/'+str(i)+'.png',temp)
    np.save(outdir+'/sal.npy', salImSeq)
    np.save(outdir+'/conf.npy', np.array(conf))
    np.save(outdir+'/indices.npy', np.array(indices))
...
#        frameGap = int(len(imPathList) / lenSeq)
    ...
    # run the algorithm
    nlc(imSeq, maxsp=args.maxsp, iters=args.iters, outdir=args.outdir, indices=indices)
    return
