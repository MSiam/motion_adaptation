...
def compute_cnn_descriptor(im, sp, spPatch=15):
    """
    ...
    for i in range(n):
        ...
        for j in range(boxes.shape[0]):
            ...
            cnnfeats= np.load('../../fextract_onavos/MTLMotion/feats/breakdance/%05d.npy'%i)
            cnnPatch = cnnfeats[0, ymin//4:ymax//4, xmin//4:xmax//4,:]
            cnnPatch= cnnPatch.mean(axis=(0,1))

            #regions[count, :] = np.hstack((
            #    hogF, colHist, [boxes[j, 1] * 1. / h, boxes[j, 0] * 1. / w]))
            regions[count,:]= cnnPatch
            count += 1
        frameEnd[i] = count - 1
...
def compute_saliency(imSeq, flowSz=100, flowBdd=12.5, flowF=3, flowWinSz=10,
    ...
    return salImSeqOrig, confs, valid
...
def nlc(imSeq, maxsp, iters, outdir, suffix='',
    ...
    if not doload:
        ...
        regions, frameEnd = compute_cnn_descriptor(imSeq, sp)
        ...
        salImSeq, confs, valid = compute_saliency(imSeq, flowBdd=12.5, flowDirBins=20,
                                        redirect=redirect)
        print('Validity of sequence is ', valid)
    ...
    if doload:
        ...
        confs = np.load(outdir + '/confs.npy')
        valid = np.load(outdir + '/valid.npy')
    if dosave:
